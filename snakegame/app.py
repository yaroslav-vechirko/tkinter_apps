...
class Snake(tk.Canvas):
    ...
    def load_assets(self):
        try:
            bundl_dir = getattr(sys, "_MEIPASS", path.abspath(path.dirname(__file__)))
            path_to_snake = path.join(bundl_dir, "assets", "snake.png")
            path_to_food = path.join(bundl_dir, "assets", "food.png")
    ...
    def on_key_press(self, e):
        ...
        oposites = ({'Up', 'Down'}, {'Left', 'Right'})
        if (new_direction in all_direction and {new_direction, self.direction} not in oposites):
            self.direction = new_direction

    def check_food_colisions(self):
